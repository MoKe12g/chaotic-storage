...
pub(crate) async fn get_transaction(app_state: &State<api::AppStatePointer>,
    ...
    match query_as!(Transaction, "SELECT * FROM transactions WHERE id BETWEEN ?1 AND ?2;", start, end).fetch_all(app_state.get_storage_system().get_database()).await {
        Ok(result) => {
            Ok(Json(result))
        }
...
pub async fn post_transaction(app_state: &State<api::AppStatePointer>, input: Json<Transaction>) -> Result<Json<String>, BadRequest<Json<MessageResponse>>> {
    let app_state = app_state.lock().await;
...
pub async fn delete_transaction(app_state: &State<api::AppStatePointer>, id: i64) -> Result<Json<Transaction>, BadRequest<Json<MessageResponse>>> {
    let app_state = app_state.lock().await;
    match Transaction::from(app_state.get_storage_system(), id).await {
        Ok(result) => {
            match result {
...
pub async fn count_transaction_entries(app_state: &State<api::AppStatePointer>) -> Result<Json<EntriesCountResponse>, BadRequest<Json<MessageResponse>>> {
    let result = query_as!(EntriesCountResponse, "SELECT COUNT(id) AS count, 'transactions' AS 'table' FROM transactions;").fetch_optional(app_state.lock().await.get_storage_system().get_database()).await;
    match result {
        Ok(result) => {
            match result
            {
